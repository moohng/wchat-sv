  ...
  if (req.query && req.query.username) {
    ...
    UserModel.find(req.query, fields).exec()
      .then(docs => {
        ...
        // 判断是否在线
        const online = Object.keys(req.app.user).includes(req.query.username as string);
        // 判断是否是好友关系
        const friendly = docs[0]?.friends?.includes(req.session.username as string);
    ...
    delete user_obj[req.session.username as string];
